...
class Runner():
    ...
    def __init__(self, args, config):
        ...
        # Google-Sheets experiment logging is optional: enabled only when a
        # service-account JSON is passed via --json_file (requires gspread).
        if args.json_file and gspread is not None:
        ...
        while pbar.n < pbar.total:
            ...
            for data in tqdm(dataloader, dynamic_ncols=True, desc='train'):
                ...
                try:
                    ...
                    #     flop_analysis = FlopCountAnalysis(self.upstream, example_input)
                ...
                if global_step % self.config['runner']['save_step'] == 0 or pbar.n == pbar.total -1:
                    ...
                    if global_step % 10000 == 0:
                        perm_name = f'permanent-states-{global_step}.ckpt'
                        perm_path = os.path.join(self.args.expdir, perm_name)
                        tqdm.write(f'[Runner] - Save permanent checkpoint to: {perm_path}')
                        torch.save(all_states, perm_path)
