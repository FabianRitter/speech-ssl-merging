...
class DistillerBuilder(nn.Module):
    ...
    def __init__(self, options, config, verbose=False):
        ...
        if config is not None:
            ...
            # [ORIGINAL CODE - before Up_Config fix]
            # self.config = self.all_states["Config"]
            # [END ORIGINAL CODE]
            #
            # Prefer Up_Config (full upstream architecture config) over Config
            # (runner config whose model key may only have override params).
            # This matches the ddp_branch behavior and avoids shape mismatches
            # when loading checkpoints trained in the ddp_branch codebase.
            if "Up_Config" in self.all_states:
                self.config = self.all_states["Up_Config"]
                print("[DistillerBuilder] - Loaded config from checkpoint 'Up_Config' (full upstream config)")
            else:
                self.config = self.all_states["Config"]
                print("[DistillerBuilder] - Loaded config from checkpoint 'Config' (no 'Up_Config' found)")
        ...
        # [END ORIGINAL CODE]
        #
        # Try 'multi_distiller' key first, fall back to 'distiller' for
        # checkpoints trained with the single-teacher distiller codebase.
        if "multi_distiller" in self.config:
            print(f"[DistillerBuilder] - Using 'multi_distiller' config key")
            self.model_config = MultiDistillerConfig(self.config["multi_distiller"])
        elif "distiller" in self.config:
            print(f"[DistillerBuilder] - 'multi_distiller' key not found, falling back to 'distiller' config key")
            self.model_config = MultiDistillerConfig(self.config["distiller"])
        else:
            raise KeyError("[DistillerBuilder] - Neither 'multi_distiller' nor 'distiller' key found in checkpoint config")
